...
신경망의 구조는 해당 논문을 소개한 블로그에서 참고하여 코드로 구현하였음

## data set
RCNN 모델을 이용한 부정맥 신호 인식에 관한 논문의 저자가 가공해놓은 데이터셋 이용하였음  
...
- 5개의 클래스 중 모델의 추정 클래스와 label의 클래스가 같은지만 고려하며 정확도를 평가하였음. 따라서 f1 score등 더 정량적인 모델의 평가 필요 
